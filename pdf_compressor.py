...
def compress(input_file_path, output_file_path, power=0):
    ...
    subprocess.call([gs,
                     ...
                     '-dPDFSETTINGS={quality}'.format(quality=quality[power]),
                     '-dNOPAUSE',
                     '-dQUIET',
                     '-dBATCH',
                     '-dColorConversionStrategy=/Gray',
                     '-dProcessColorModel=/DeviceGray',
                     f"-dColorImageResolution={compress_level}",
                     f"-dGrayImageResolution={compress_level}",
                     f"-dMonoImageResolution={compress_level}",
